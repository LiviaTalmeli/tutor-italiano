...
if __name__ == "__main__":
    # Inicia o agendador numa linha de execução paralela
    thread_agendador = threading.Thread(target=rodar_agendador, daemon=True)
    thread_agendador.start()

    print("🤖 Bot rodando! Monitorando grupo, protegendo contra loops e agendador ativo...")
    # O infinity_polling evita que o bot caia se a conexão de internet piscar
  # Função para rodar o bot do Telegram
def run_bot():
    print("Bot do Telegram iniciado!")
    bot.infinity_polling()

# Função para rodar o mini servidor web do Flask (para o Render)
def run_web():
    app.run(host='0.0.0.0', port=10000)

if __name__ == '__main__':
    # Inicia o bot do Telegram em uma thread paralela
    threading.Thread(target=run_bot, daemon=True).start()
    
    # Inicia o Flask no fluxo principal
    run_web()